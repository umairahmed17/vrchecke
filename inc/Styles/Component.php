<?php
/**
 * VRCHECKE\VRCHECKE\Styles\Component class
 *
 * @package vrchecke
 */

namespace VRCHECKE\VRCHECKE\Styles;

use function VRCHECKE\VRCHECKE\vrchecke;
use VRCHECKE\VRCHECKE\Component_Interface;
use VRCHECKE\VRCHECKE\Templating_Component_Interface;

/**
 * Class for managing stylesheets.
 *
 * Exposes template tags:
 * * `vrchecke()->print_styles()`
 */
class Component implements Component_Interface, Templating_Component_Interface {
	...
	/**
	 * Gets template tags to expose as methods on the Template_Tags class instance, accessible through `vrchecke()`.
	 *
	...
	public function action_enqueue_styles() {
		...
		if ( ! empty( $google_fonts_url ) ) {
			wp_enqueue_style( 'vrchecke-fonts', $google_fonts_url, array(), null ); // phpcs:ignore WordPress.WP.EnqueuedResourceParameters.MissingVersion
		}
		...
		foreach ( $css_files as $handle => $data ) {
			...
			$version = vrchecke()->get_asset_version( $css_dir . $data['file'] );
	...
	public function filter_resource_hints( array $urls, string $relation_type ): array {
		if ( 'preconnect' === $relation_type && wp_style_is( 'vrchecke-fonts', 'queue' ) ) {
			$urls[] = array(
		...
		$handles   = array_filter(
			...
			function ( $handle ) use ( $css_files ) {
				...
				if ( ! $is_valid ) {
					...
					_doing_it_wrong( __CLASS__ . '::print_styles()', esc_html( sprintf( __( 'Invalid theme stylesheet handle: %s', 'vrchecke' ), $handle ) ), 'VRCHECKE 2.0.0' );
				}
				return $is_valid;
			}
		);
	 ...
	 * {@see 'vrchecke_preloading_styles_enabled'} filter can be used to tweak the return value.
	 *
	...
	protected function preloading_styles_enabled() {
		$preloading_styles_enabled = ! vrchecke()->is_amp();
		 ...
		 */
		return apply_filters( 'vrchecke_preloading_styles_enabled', $preloading_styles_enabled );
	}
	...
	protected function get_css_files(): array {
		...
		$css_files = array(
			'vrchecke-global'        => array(
				'file'   => 'global.min.css',
				'global' => true,
			),
			'vrchecke-comments'      => array(
				'file'             => 'comments.min.css',
				'preload_callback' => function () {
					return ! post_password_required() && is_singular() && ( comments_open() || get_comments_number() );
				},
			),
			'vrchecke-content'       => array(
				'file'             => 'content.min.css',
				'preload_callback' => '__return_true',
			),
			'vrchecke-sidebar'       => array(
				'file'             => 'sidebar.min.css',
				'preload_callback' => function () {
					return vrchecke()->is_primary_sidebar_active();
				},
			),
			'vrchecke-widgets'       => array(
				'file'             => 'widgets.min.css',
				'preload_callback' => function () {
					return vrchecke()->is_primary_sidebar_active();
				},
			),
			'vrchecke-front-page'    => array(
			...
			'vrchecke-vrchecke-form' => array(
				'file'             => 'vrchecke-form.min.css',
				'preload_callback' => function () {
					global $template;
					return 'form.php' === basename( $template );
				},
			),
			'vrchecke-login'         => array(
				'file'             => 'page-login.min.css',
				'preload_callback' => function () {
					global $template;
					return 'page-login.php' === basename( $template );
				},
			),
			'vrchecke-portal'        => array(
				'file'             => 'page-customer-portal.min.css',
				'preload_callback' => function () {
					global $template;
					return 'page-customer-portal.php' === basename( $template );
				},
			),
			'intlTelInput'         => array(
		 ...
		 */
		$css_files = apply_filters( 'vrchecke_css_files', $css_files );
	...
	protected function get_google_fonts(): array {
		 ...
		 */
		$this->google_fonts = (array) apply_filters( 'vrchecke_google_fonts', $google_fonts );
